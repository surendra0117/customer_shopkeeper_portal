...
app.get("/:userid/showmessages", function (req, res) {
  ...
  loginmodel.findById(a, function (err, user) {
    ...
    messagemodel.find({ receiver: user.name }, function (err, transaction) {
      // messages
      if (err) {
        console.log(err);
      } else {
  ...
  if (req.body.type == "customer") {
    customermodel.findOne({ username: us }, function (err, customer) {
      if (pass == customer.password) {
        ...
        const locationFilePath = path.join(__dirname, "views/homepage.html");
        res.sendFile(locationFilePath);
    ...
    if (err) {
      ...
      owner.items.forEach(function (item) {
        if (item._id == a) {
          ...
          itemmodel.findOne(
            { itemname: eitem, owner: owner.name },
            function (err, item) {
              if (err) {
                console.log(err);
              } else {
...
app.post("/:cusid/search", function (req, res) {
  ...
  itemmodel.find({ itemname: b }, function (err, item) {
    //  item search
    if (err) {
      console.log("peter");
      console.log(err);
...
app.post("/:senderid/sendmessage", function (req, res) {
  ...
  var snr;
  loginmodel.findById(a, function (err, sender) {
    //  send message
    snr = sender.name;
